...
for (const [name, version] of [
  ...
  if (version !== "1.4.4") throw new Error(`${name} release version must be 1.4.4`);
}
...
if (!popupHtml.includes('id="version">v1.4.4</span>') || !popupJs.includes('"v1.4.4"')) {
  throw new Error("popup release metadata must be v1.4.4");
}
...
for (const [name, source] of [
  ...
  if (!source.includes('"1.4.4"')) throw new Error(`${name} release metadata must be v1.4.4`);
}
...
if (!changelog.includes("## v1.4.4 — 2026-07-20")) throw new Error("CHANGELOG must contain the v1.4.4 release");
