...
test("release metadata is synchronized for v1.4.2", async () => {
  ...
  assert.match(popupHtml, /<span id="version">v1\.4\.2<\/span>/u);
  assert.match(popupJs, /"v1\.4\.2"/u);
  assert.match(readme, /Version-v1\.4\.2-/u);
  assert.match(readme, /alt="Version v1\.4\.2"/u);
  assert.match(bridgeClient, /BRIDGE_CLIENT_VERSION = "1\.4\.2"/u);
  assert.match(nativeHost, /HOST_VERSION = "1\.4\.2"/u);
...
test("v1.4 documentation covers every new workflow, schedule, and WebMCP contract", async () => {
  ...
  assert.match(changelog, /## v1\.4\.2 — 2026-07-16/u);
