...
test("release metadata is synchronized for v1.5.0", async () => {
  ...
  assert.match(popupHtml, /<span id="version">v1\.5\.0<\/span>/u);
  assert.match(popupJs, /"v1\.5\.0"/u);
  assert.match(readme, /Version-v1\.5\.0-/u);
  assert.match(readme, /alt="Version v1\.5\.0"/u);
  assert.match(bridgeClient, /BRIDGE_CLIENT_VERSION = "1\.5\.0"/u);
  assert.match(nativeHost, /HOST_VERSION = "1\.5\.0"/u);
  assert.equal(packageJson.dependencies["@opencode-ai/plugin"], "1.18.15");
  assert.equal(packageLock.packages[""].dependencies["@opencode-ai/plugin"], "1.18.15");
  assert.equal(packageLock.packages["node_modules/@opencode-ai/plugin"].version, "1.18.15");
  assert.equal(packageLock.packages["node_modules/@opencode-ai/sdk"].version, "1.18.15");
});
...
test("v1.4 documentation covers every new workflow, schedule, and WebMCP contract", async () => {
  ...
  assert.match(changelog, /## v1\.5\.0 — 2026-08-11/u);
